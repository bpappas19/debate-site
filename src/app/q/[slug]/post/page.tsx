...
export default function PostTakePage({
  ...
  const [question, setQuestion] = useState(
    typeof params === "object" && "slug" in params
      ? mockQuestions.find((x) => x.slug === params.slug) || null
      : null
  );
  ...
  useEffect(() => {
    if (params instanceof Promise) {
      params.then((resolved) => {
        const found = mockQuestions.find((x) => x.slug === resolved.slug);
        setQuestion(found || null);
      });
    } else {
      const found = mockQuestions.find((x) => x.slug === params.slug);
      setQuestion(found || null);
    }
  }, [params]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    console.log("Posting take:", payload);
    alert("Take posted! (Mock - no backend yet)");
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-4">
              ...
              <textarea
                ...
                id="argument"
                placeholder="Build your case. Mention key points, evidence, or reasoning to persuade others. High-quality arguments earn more upvotes..."
                rows={6}
                value={argument}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-stretch gap-2">
                ...
                <button
                  ...
                  onClick={() => {
                    // In a real app, this would add the source to a list
                    if (sourceUrl.trim()) {
            ...
            <div className="pt-6 border-t border-[#e7ebf3] dark:border-[#2d3748] flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-3">
                <div className="flex -space-x-2">
                  <div className="size-8 rounded-full border-2 border-white dark:border-[#1a2133] bg-[#135bec]/20 flex items-center justify-center text-sm">
                    👤
                  </div>
                  <div className="size-8 rounded-full border-2 border-white dark:border-[#1a2133] bg-[#135bec] flex items-center justify-center text-[10px] font-bold text-white uppercase">
                    Me
                  </div>
                </div>
